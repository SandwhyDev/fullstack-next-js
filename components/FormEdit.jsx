...
const FormEdit = ({ id, nama_lengkap, alamat, email, nomor_hp, usia }) => {
  const handleSubmit = (e) => {
    e.preventDefault();

    const { nama_lengkap, alamat, email, nomor_hp, usia } = e.target;

    axios("/api/mahasiswa/update", {
      method: "PUT",
      data: {
        id: id,
  ...
  return (
    <div className="w-screen h-screen bg-black/30 flex flex-col items-center justify-center fixed top-0 left-0">
      <form
      ...
      >
        ...
        <button
          className="p-2 bg-red-500 uppercase text-white text-xl"
          onClick={() => {
            window.location.reload();
          }}
        >
